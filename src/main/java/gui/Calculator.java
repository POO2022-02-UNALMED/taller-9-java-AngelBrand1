...
public class Calculator extends VBox implements EventHandler<ActionEvent>{
	...
	public void handle(ActionEvent event) {
		...
		String ant = this.displayText.getText();
		boolean val = false;
        String[] op = {"*","+","/","-"};
        for(String i:op){
            if(value.contains(i)){
        ...
        if(value=="=") {
        	try {
	        	int len = (this.number1).length()+1;
	        	this.number2 = ant.substring(len);
	        	float num1 = Float.parseFloat(this.number1);
	        	float num2 = Float.parseFloat(this.number2);
	        	float resultado;
	        	switch(this.operator) {
	        	case "*":
	        		resultado = num1*num2;
	        		
	        		this.res = String.format("%.2f",resultado);
	        		break;
	        	case "/":
	        		resultado =num1/num2;
	        		resultado = (resultado%1==0)?Math.round(resultado):resultado;
        		...
        		this.res= "operación invalida";
        	}

        	
        }
		this.displayText.setText((value!="=")?ant+value:this.res);
